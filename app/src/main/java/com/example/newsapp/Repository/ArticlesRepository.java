...
public class ArticlesRepository {
    ...
    /**
     * Enhanced test method to debug all aspects of liked articles
     */
    public void debugLikedArticlesComprehensive(String userId) {
        Log.d("ArticlesRepository", "=== COMPREHENSIVE LIKED ARTICLES DEBUG ===");
        Log.d("ArticlesRepository", "Target User ID: " + userId);

        // Step 1: Check user authentication
        FirebaseUser currentUser = FirebaseAuth.getInstance().getCurrentUser();
        if (currentUser != null) {
            Log.d("ArticlesRepository", "Current user ID: " + currentUser.getUid());
            Log.d("ArticlesRepository", "User email: " + currentUser.getEmail());
            Log.d("ArticlesRepository", "User matches target: " + currentUser.getUid().equals(userId));
        } else {
            Log.e("ArticlesRepository", "NO USER LOGGED IN!");
        }

        // Step 2: Check ALL articles to see if any have liked_by field
        Log.d("ArticlesRepository", "--- Checking ALL articles for liked_by field ---");
        db.collection("articles")
                .limit(20)
                .get()
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    Log.d("ArticlesRepository", "Total articles in database: " + queryDocumentSnapshots.size());

                    int articlesWithLikedBy = 0;
                    int articlesLikedByUser = 0;
                    ...
                    for (DocumentSnapshot doc : queryDocumentSnapshots.getDocuments()) {
                        ...
                        Log.d("ArticlesRepository", "Article: " + articleId);
                        Log.d("ArticlesRepository", "  Title: " + title);
                        Log.d("ArticlesRepository", "  Status: " + status);
                        Log.d("ArticlesRepository", "  Has liked_by: " + doc.contains("liked_by"));

                        if (likedBy != null) {
                            articlesWithLikedBy++;
                            Log.d("ArticlesRepository", "  liked_by: " + likedBy.toString());
                            if (likedBy.contains(userId)) {
                                articlesLikedByUser++;
                                Log.d("ArticlesRepository", "  *** USER LIKED THIS ARTICLE! ***");
                            }
                        } else {
                            Log.d("ArticlesRepository", "  liked_by: null");
                        }
                        Log.d("ArticlesRepository", "  ---");
                    }

                    Log.d("ArticlesRepository", "=== SUMMARY ===");
                    Log.d("ArticlesRepository", "Articles with liked_by field: " + articlesWithLikedBy);
                    Log.d("ArticlesRepository", "Articles liked by user: " + articlesLikedByUser);
                })
                .addOnFailureListener(e -> {
                    Log.e("ArticlesRepository", "Error checking articles: " + e.getMessage());
                });

        // Step 3: Test the exact query used in getArticlesLikedByUser
        Log.d("ArticlesRepository", "--- Testing exact liked articles query ---");
        db.collection("articles")
                .whereArrayContains("liked_by", userId)
                .get()
                .addOnSuccessListener(queryDocumentSnapshots -> {
                    Log.d("ArticlesRepository", "Direct query result: " + queryDocumentSnapshots.size() + " articles");
                    for (DocumentSnapshot doc : queryDocumentSnapshots.getDocuments()) {
                        Log.d("ArticlesRepository", "Direct query article: " + doc.getString("title"));
                    }
                })
                .addOnFailureListener(e -> {
                    Log.e("ArticlesRepository", "Direct query failed: " + e.getMessage());
                });
    }
